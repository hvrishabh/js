...
const getCountryData = function (country) {
  ...
  fetch(`https://restcountries.com/v2/name/${country}`)
    .then(
      ...
      // err => alert(err) // handling the error //or// catching the error
    )
    .then(data => {
      renderCountry(data[0]);
    ...
    // .catch(err => alert(err)); // this catch method will catch all the errors no matter , where it has occured in the above chain
    .catch(err => {
      console.log(`${err} 👀👀👀👀`);
